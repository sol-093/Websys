...
function handleUpdateMyOrgAction(PDO $db, array $user): void
{
    ...
    try {
        withTransaction($db, static function () use ($db, $name, $description, $logoPath, $logoCropX, $logoCropY, $logoZoom, $org): void {
        ...
        if (isset($uploadedLogo) && $uploadedLogo !== false && trim((string) ($org['logo_path'] ?? '')) !== '' && trim((string) ($org['logo_path'] ?? '')) !== $uploadedLogo) {
            deleteStoredUpload((string) $org['logo_path']);
        }
        setFlash('success', 'Organization details updated.');
    } catch (Throwable $e) {
        if (isset($uploadedLogo) && $uploadedLogo !== false) {
            deleteStoredUpload((string) $uploadedLogo);
        }
        setFlash('error', 'Unable to update organization details.');
    }
    redirect('?page=my_org_manage&org_id=' . (int) $org['id']);
}
...
{
    requirePermission('manage_own_organization');
    $announcements = new Involve\Repositories\AnnouncementRepository($db);
    $selectedOrgId = (int) ($_POST['org_id'] ?? 0);
    $org = getOwnedOrganizationById((int) $user['id'], $selectedOrgId);
    if (!$org) {
    ...
    try {
        withTransaction($db, static function () use ($announcements, $org, $title, $content, $label, $durationDays, $expiresAt): void {
            $announcements->create((int) $org['id'], $title, $content, $label, $durationDays, $expiresAt);
        });
...
function handleDeleteAnnouncementAction(PDO $db, array $user): void
{
    ...
    if ($org) {
        try {
            withTransaction($db, static function () use ($announcements, $id, $org): void {
                $announcements->deleteForOrganization($id, (int) $org['id']);
            });
            setFlash('success', 'Announcement deleted.');
        } catch (Throwable $e) {
            setFlash('error', 'Unable to delete announcement.');
        }
    }
...
function handlePinAnnouncementAdminAction(PDO $db, array $user): void
{
    ...
    try {
        withTransaction($db, static function () use ($announcements, $announcementId): void {
            if (!$announcements->exists($announcementId)) {
                throw new RuntimeException('Announcement not found.');
            }

            $announcements->pinExclusive($announcementId);
        });
...
function handleUnpinAnnouncementAdminAction(PDO $db, array $user): void
{
    ...
    try {
        $announcements->unpin($announcementId);
...
function handleAddTransactionAction(PDO $db, array $user, array $config): void
{
    ...
    try {
        withTransaction($db, static function () use ($transactions, $org, $type, $amount, $description, $transactionDate, $receiptPath): void {
            $transactions->create((int) $org['id'], $type, $amount, $description, $transactionDate, $receiptPath);
        });
        ...
        setFlash('error', 'Unable to save transaction.');
    }
    redirect('?page=my_org_finance&org_id=' . (int) $org['id'] . '#tx-history');
}
...
function handleUpdateTransactionAction(PDO $db, array $user): void
{
    ...
    try {
        withTransaction($db, static function () use ($transactions, $txId, $org, $user, $type, $amount, $description, $transactionDate): void {
            $transactions->requestUpdate($txId, (int) $org['id'], (int) $user['id'], $type, $amount, $description, $transactionDate);
        });
        setFlash('success', 'Update request sent to admin for approval.');
    } catch (Throwable $e) {
        setFlash('error', 'Unable to send update request.');
    }
    redirect('?page=my_org_finance&org_id=' . (int) $org['id'] . '#tx-history');
}
...
function handleDeleteTransactionAction(PDO $db, array $user): void
{
    ...
    if ($org) {
        ...
        if ($transactions->hasPendingChangeRequest($txId, 'delete')) {
            setFlash('error', 'A delete request for this transaction is already pending.');
            redirect('?page=my_org_finance&org_id=' . (int) $org['id'] . '#tx-history');
        }

        try {
            withTransaction($db, static function () use ($transactions, $txId, $org, $user): void {
                $transactions->requestDelete($txId, (int) $org['id'], (int) $user['id']);
            });
            setFlash('success', 'Delete request sent to admin for approval.');
        } catch (Throwable $e) {
            setFlash('error', 'Unable to send delete request.');
        }
    }
...
function handleExportTransactionsAction(PDO $db, array $user): void
{
    ...
    $templateImage = $readPdfPngTemplate(dirname(__DIR__, 3) . '/uploads/assets/pdftemplate.png');
